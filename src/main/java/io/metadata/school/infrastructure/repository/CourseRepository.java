...
public interface CourseRepository extends JpaRepository<Course, Integer> {
	...
	@Query(value = "SELECT s.id, s.name FROM student s INNER JOIN course_registration cr ON (s.id = cr.student_id) INNER JOIN course c ON (c.id = cr.course_id) WHERE s.id = :id ", nativeQuery = true)
	List<Course> findByStudentId(@Param("id") Integer id);

	@Query(value = "SELECT c.id, c.name FROM course c LEFT JOIN course_registration cr ON (c.id = cr.student_id) WHERE cr.course_id IS NULL", nativeQuery = true)
